...
class Task():
  ...
  def oracle(self, env):
    ...
    def act(obs, info):  # pylint: disable=unused-argument
      ...
      # ad-hoc
      # fixture_pose = p.getBasePositionAndOrientation(self.fixture_id)
      # obj_euler = utils.quatXYZW_to_eulerXYZ(fixture_pose[1])
      # if (np.abs(obj_euler[0] - 1.57) > 0.2):
      #       pick_pose = (fixture_pose[0], (0, 0, 0, 1))
      #       place_pix = (fixture_pose[0][0], fixture_pose[0][1]-0.06, 0.08)
      #       # self.markers.append(utils.SphereMarker(place_pix))
      #       place_rot = utils.eulerXYZ_to_quatXYZW((np.pi/2, 0, 0))
      #       place_pose = (place_pix, place_rot)
      #       return {'pose0': pick_pose, 'pose1': place_pose, 'contact': False}
      ...
      # ad-hoc
      # targ_pose = (fixture_pose[0], (0, 0, 0, 1))
    ...
    if metric == 'pose':
      ...
      for i in range(len(objs)):
        ...
        for j in targets_i:
          target_pose = targs[j]
          # ad-hoc
          # target_pose = p.getBasePositionAndOrientation(self.fixture_id)
